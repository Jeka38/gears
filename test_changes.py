...
class TestBotChanges(unittest.TestCase):
    ...
    def test_direct_url_download(self):
        ...
        self.bot.file_transfer.download_from_url.assert_called_once()
        args, kwargs = self.bot.file_transfer.download_from_url.call_args
        self.assertEqual(args[0], 'https://example.com/image.png')
    ...
    def test_direct_url_php_blocking(self):
        ...
        loop.run_until_complete(ft_plugin.download_from_url('https://example.com/shell.php?cmd=ls', 'safe.txt', peer_jid))
        self.bot.send_message.assert_called_with(mto=peer_jid, mbody="❌ Ошибка: Загрузка PHP-файлов запрещена!", mtype='chat')
